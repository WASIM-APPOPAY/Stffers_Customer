        ...
        compressionTask = new AsyncTask<File, Void, String>() {
            ...
            protected String doInBackground(File... files) {
                if (child.equals("images")) {
                    try {
                        String mPath = context.getCacheDir().getPath() + File.separator + "images";
                        String compress = SiliCompressor.with(context).compress(files[0].toString(), new File(mPath));
                        //File tempFile = new Compressor(context).setQuality(70).compressToFile(files[0]);
                        File tempFile = new File(compress);
